def plot_horizons_same_lags(
    ...
    ax.grid(True)
    plt.tight_layout()
    os.makedirs(f"results/{model}_{target_level}_{predicted_by}", exist_ok=True)
    plt.savefig(f"results/{model}_{target_level}_{predicted_by}/{target_level}_{predicted_by}_{lags}.png")
