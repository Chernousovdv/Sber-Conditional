...
class ClusterForecaster:
    ...
    def train(self,
              ...
              train_end: pd.Timestamp = None,
              model_type_naive: str = "NaiveSeasonal",  # default to a baseline
              model_type: str = "nbeats",  # default to a baseline
              **model_kwargs):
        """
        ...
        model_type_naive_lower = model_type_naive.lower()
        ...
        if model_type_naive_lower == "naiveseasonal":
            # user should provide K (seasonal period), default to 12 for monthly
            K = model_kwargs.pop("K", 12)
            model_naive = NaiveSeasonal(K=K)
            uses_covariates = False

        elif model_type_naive_lower == "naivedrift":
            model_naive = NaiveDrift()
            uses_covariates = False

        elif model_type_naive_lower == "naivemean":
            model_naive = NaiveMean()
            uses_covariates = False

        elif model_type_naive_lower == "naivemovingaverage":
            window = model_kwargs.pop("window", None)
            if window is None:
                raise ValueError("NaiveMovingAverage requires 'window' argument (int).")
            model_naive = NaiveMovingAverage(window=window)
            uses_covariates = False

        elif model_type_naive_lower == "exponentialsmoothing" or model_type_lower == "es":
            model_naive = ExponentialSmoothing()
            uses_covariates = False

        elif model_type_naive_lower == "theta":
            model_naive = Theta()
            uses_covariates = False
        ...
        # fit model
        model_naive.fit(series=target_ts, verbose=True)
        model.fit(series=target_ts, past_covariates=cov_ts, verbose=True)

        self.model_naive = model
        self.model = model
    ...
    def forecast(self, n: int = 12, covariates_future: Optional[pd.DataFrame] = None):
        ...
        pred_naive = self.model_naive.predict(n=n)

        return pred_naive, pred
    ...
    def backtest(self,
        ...
        # forecast
        pred_ts_naive, pred_ts = self.forecast(n=forecast_horizon, covariates_future=cov_future)
        ...
        mape_val_naive = mape(actual_ts, pred_ts_naive)
        mae_va_naive = mae(actual_ts, pred_ts_naive)
        rmse_val_naive = rmse(actual_ts, pred_ts_naive)
        # (your plot function)
        plot_preds_on_curve(pred_ts.to_dataframe(),
                            ...
                            pred_ts_naive=pred_ts_naive,
                            mape_val_naive=mape_val_naive)

        metrics = {"MAPE": mape_val, "MAE": mae_val, "RMSE": rmse_val,
                   "MAPE NAIVE": mape_val_naive, "MAE NAIVE": mae_va_naive, "RMSE NAIVE": rmse_val_naive}
        ...
        preds_naive_df = pred_ts_naive.to_dataframe().rename(columns={f"{target_cluster}{self.target_col_suffix}": f"preds_naive_{target_cluster}{self.target_col_suffix}"})
        actual_df = actual_ts.pd_dataframe().rename(columns={f"{target_cluster}{self.target_col_suffix}": f"true_{target_cluster}{self.target_col_suffix}"})
        result_df = pd.concat([to_dataframe, preds_df, preds_naive_df], axis=1)
        ...
        return preds_naive_df, preds_df, actual_df, metrics
